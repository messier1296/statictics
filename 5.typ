...
$K(>=2)$個の結果のうちいずれか一つが起こる確率を考える.
結果$j$が起こる確率を$p_j$とする.
($p_1 dots p_K >0,sum p_i = 1$)
この試行を独立に$n$回行うとき結果$j$の起こる確率を$X^((i))$とする.
このとき$X:(X^((1)) dots X^((K)))$の従う分布を*多項分布*と呼び,$M(n;p_1dots p_K)$と表す.
$sum X^((i)) = n$が常に成り立つことに注意する.
特に$K = 2$のときの場合は二項分布になるので,多項分布は二項分布の一般化ということができる.

多項分布$M(n;p_1 dots p_K)$の確率関数は
$
  P(X^((1)) = x^((1))dots X^((K)) = x^((K))) = n!/(x^((1)) !dots x^((K)) ! ) p_1^(x^((1))) dots p_K^(x^((K)))\
  x^(j) in {1 dots K}(0 <= j <= K),
  #h(2em)
  x^((1)) + dots x^((K)) = n
$
となる.
...
多項分布の期待値は
$
  E[X_1] &= sum_(x_1 = 0)^n x_1 P(X^((1)) = x_1 dots X^((K)) = x_k) \
  &= sum_(x_1=1)^n x_1 n!/(x_1 !dots x_K ! ) p_1^(x_1) dots p_K^(x_K)
  &= n p_1 sum_(x_1=1)^n (n-1)!/((x_1 - 1)! x_2 ! dots x_K ! ) p_1^(x_1 - 1) p_2^(x_2) dots p_K^(x_K) \
  &= n p_1 (p_1 + p_2 + dots + p_K)^(n-1) \
  &= n p_1
$
ここで, 2行目の等号はパラメータ$n-1$の多項分布の確率の総和が$1$になること(あるいは多項定理)を用いた.
同様にして任意の$i$について$E[X^((i))] = n p_i$が成り立つ.

分散を求めるために, まず階乗モーメント$E[X^((i))(X^((i))-1)]$を計算する.
$
  E[X^((i))(X^((i))-1)] &= sum_(x^((i))=0)^n x^((i))(x^((i))-1) P(X^((1)) = x^((1)) dots X^((K)) = x^((K))) \
  &= sum_(x^((i))=2)^n x^((i))(x^((i))-1) n!/(x^((1)) !dots x^((K)) ! ) p_1^(x^((1))) dots p_K^(x^((K))) \
  &= n(n-1)p_i^2 sum_(x^((i))=2)^n (n-2)!/(x^((1)) ! dots (x^((i))-2)! dots x^((K)) ! ) p_1^(x^((1))) dots p_i^(x^((i))-2) dots p_K^(x^((K))) \
  &= n(n-1)p_i^2 (p_1 + dots + p_K)^(n-2) \
  &= n(n-1)p_i^2
$
これを用いると, $E[(X^((i)))^2]$は以下のように求まる.
$
  E[(X^((i)))^2] & = E[X^((i))(X^((i))-1)] + E[X^((i))] \
                 & = n(n-1)p_i^2 + n p_i
$
よって分散$V[X^((i))]$は
$
  V[X^((i))] & = E[(X^((i)))^2] - (E[X^((i))])^2 \
             & = n(n-1)p_i^2 + n p_i - (n p_i)^2 \
             & = n^2 p_i^2 - n p_i^2 + n p_i - n^2 p_i^2 \
             & = n p_i (1 - p_i)
$
となる.
なお, 多項分布の周辺分布は二項分布$B(n, p_i)$に従うため, この結果は二項分布の分散と一致している.

多項分布の積率母関数$M(theta_1, dots, theta_K)$を定義に従って求める.
$theta = (theta_1, dots, theta_K)$とする.
$
  M(theta) &= E[exp(sum_(j=1)^K theta_j X^((j)))] \
  &= sum_(x^((1)) + dots + x^((K)) = n) n!/(x^((1)) !dots x^((K)) ! ) p_1^(x^((1))) dots p_K^(x^((K))) exp(sum_(j=1)^K theta_j x^((j))) \
  &= sum_(x^((1)) + dots + x^((K)) = n) n!/(x^((1)) !dots x^((K)) ! ) (p_1 e^(theta_1))^(x^((1))) dots (p_K e^(theta_K))^(x^((K)))
$
多項定理により, これは以下のようにまとめられる.
$
  M(theta) = (p_1 e^(theta_1) + p_2 e^(theta_2) + dots + p_K e^(theta_K))^n
$
