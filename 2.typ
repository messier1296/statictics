...
$s = 1$のとき$G(1) = 1$より$|s| <= 1$のとき常に$G(s)$は収束する.それ以外の範囲では収束が保証されないがここでは右辺が収束すると仮定する.()
...
$
m(theta) = integral_infinity ^infinity e^(t x) d x 
$
という積分で得られる.このような違いから両者は同じ母関数ではあるがモーメント母関数は連続確率変数に用いられる.
